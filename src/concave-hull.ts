...
/**
 * Typescript implementation of the concave hull algorithm as described in<br/><br/>
 *
...
class ExtRTree extends RTree
{
  ...
  public toBBox(a: PointNode): BBox
  {
    if (isPoint2D(a))
    {
      return {
...
function compareByX(a: Point2D, b: Point2D): number
{
...
function noIntersections(a: Point2D, b: Point2D, segTree: ExtRTree): boolean
{
  ...
  for (i = 0; i < len; ++i)
  {
    if (utils.intersects(edges[i].p, (edges[i].next as RTreeNode).p, a, b)) {
      return false;
    }
  }
...
function fastConvexHull(points: Array<Point2D>): Array<Point2D>
{
